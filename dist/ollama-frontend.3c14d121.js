        ...
        kTBnD: [
            function (require, module, exports, __globalThis) {
                ...
                async function getResponse() {
                    ...
                    userMsg.innerHTML = `<div class="user-message"><div class="user-icon">\u{1F642}</div><div class="user-prompt">${prompt}</div></div>`;
                    chatLog.appendChild(userMsg);
                    const botMsg = document.createElement("div");
                    botMsg.innerHTML = `<strong>\u{1F916}</strong> `;
                        ...
                        for (const line of lines)
                            try {
                                ...
                                if (data.response) {
                                    ...
                                    botMsg.innerHTML = `<div class="bot-message"><div class="bot-icon"><img src="ollemur-icon-16px.png"></div><div class="bot-prompt">${(0, _marked.marked).parse(fullReply)}</div></div>`;
                                }
                            } catch (err) {
                                console.error(
                 ...
                 */ (function (global, factory) {
                    ...
                    function edit(regex, opt = "") {
                        ...
                        const obj = {
                            replace: (name, val) => {
                                let valSource =
                                    typeof val === "string" ? val : val.source;
                                valSource = valSource.replace(
                                    other.caret,
                                    "$1",
                                );
                    ...
                    const _punctuationOrSpaceGfmStrongEm =
                        /(?!~)[\s\p{P}\p{S}]/u;
                    const _notPunctuationOrSpaceGfmStrongEm =
                        /(?:[^\s\p{P}\p{S}]|~)/u;
                    ...
                    const emStrongRDelimAstCore =
                        "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)"; // (6) a***a can be either Left or Right Delimiter
                    const emStrongRDelimAst = edit(emStrongRDelimAstCore, "gu")
                        .replace(/notPunctSpace/g, _notPunctuationOrSpace)
                        .replace(/punctSpace/g, _punctuationOrSpace)
                        .replace(/punct/g, _punctuation)
                        .getRegex();
                    const emStrongRDelimAstGfm = edit(
                        emStrongRDelimAstCore,
                        "gu",
                    )
                        .replace(
                            /notPunctSpace/g,
                            _notPunctuationOrSpaceGfmStrongEm,
                        )
                    ...
                    function splitCells(tableRow, count) {
                        ...
                        const row = tableRow.replace(
                                ...
                                (match, offset, str) => {
                                    ...
                                    if (escaped)
                                        ...
                                        return "|"; // add space before unescaped |
                                    else return " |";
                                },
                            ),
                        ...
                        for (
                            ;
                            i < cells.length;
                            i++ // leading or trailing whitespace is ignored per the gfm spec
                        )
                            cells[i] = cells[i]
                                .trim()
                                .replace(other.slashPipe, "|");
                        return cells;
                    }
                    /**
                     ...
                     */ class _Tokenizer {
                            ...
                            if (cap) {
                                ...
                                while (lines.length > 0) {
                                    ...
                                    for (i = 0; i < lines.length; i++) {
                                        ...
                                        if (
                                            this.rules.other.blockquoteStart.test(
                                                lines[i],
                                            )
                                        ) {
                        ...
                        list(src) {
                            ...
                            if (cap) {
                                ...
                                // Get next list item
                                const itemRegex =
                                    this.rules.other.listItemRegex(bull);
                                ...
                                while (src) {
                                    ...
                                    if (
                                        blankLine &&
                                        this.rules.other.blankLine.test(
                                            nextLine,
                                        )
                                    ) {
                                        raw += nextLine + "\n";
                                    ...
                                    if (!endEarly) {
                                        const nextBulletRegex =
                                            this.rules.other.nextBulletRegex(
                                                indent,
                                            );
                                        const hrRegex =
                                            this.rules.other.hrRegex(indent);
                                        const fencesBeginRegex =
                                            this.rules.other.fencesBeginRegex(
                                                indent,
                                            );
                                        const headingBeginRegex =
                                            this.rules.other.headingBeginRegex(
                                                indent,
                                            );
                                        const htmlBeginRegex =
                                            this.rules.other.htmlBeginRegex(
                                                indent,
                                            );
                                    ...
                                    if (!list.loose) {
                                        ...
                                        if (endsWithBlankLine)
                                            ...
                                            this.rules.other.doubleBlankLine.test(
                                                raw,
                                            )
                                        )
                                    ...
                                    if (this.options.gfm) {
                                        istask =
                                            this.rules.other.listIsTask.exec(
                                                itemContents,
                                            );
                                        if (istask) {
                                            ischecked = istask[0] !== "[ ] ";
                        ...
                        table(src) {
                            ...
                            const rows = cap[3]?.trim()
                                ? cap[3]
                                      .replace(
                                          this.rules.other.tableRowBlankLine,
                                          "",
                                      )
                                      .split("\n")
                                : [];
                            ...
                            for (const align of aligns) {
                                if (
                                    this.rules.other.tableAlignRight.test(align)
                                )
                                    item.align.push("right");
                                else if (
                                    this.rules.other.tableAlignCenter.test(
                                        align,
                                    )
                                )
                                    item.align.push("center");
                                else if (
                                    this.rules.other.tableAlignLeft.test(align)
                                )
                            ...
                            for (const row of rows)
                                item.rows.push(
                                    splitCells(row, item.header.length).map(
                                        (cell, i) => {
                                            return {
                                                text: cell,
                        ...
                        tag(src) {
                            ...
                            if (cap) {
                                ...
                                if (
                                    ...
                                    this.rules.other.startPreScriptTag.test(
                                        cap[0],
                                    )
                                )
                                    ...
                                    this.lexer.state.inRawBlock &&
                                    this.rules.other.endPreScriptTag.test(
                                        cap[0],
                                    )
                                )
                        ...
                        link(src) {
                            ...
                            if (cap) {
                                ...
                                if (
                                    ...
                                    if (
                                        !this.rules.other.endAngleBracket.test(
                                            trimmedUrl,
                                        )
                                    )
                                        return;
                                ...
                                if (this.options.pedantic) {
                                    ...
                                    const link =
                                        this.rules.other.pedanticHrefTitle.exec(
                                            href,
                                        );
                                    if (link) {
                                        href = link[1];
                        ...
                        emStrong(src, maskedSrc, prevChar = "") {
                            let match =
                                this.rules.inline.emStrongLDelim.exec(src);
                            if (!match) return;
                            // _ can't be between two alphanumerics. \p{L}\p{N} includes non-english alphabet/numbers as well
                            if (
                                match[3] &&
                                prevChar.match(
                                    this.rules.other.unicodeAlphaNumeric,
                                )
                            )
                                return;
                            ...
                            if (
                                ...
                                while (
                                    ...
                                    const lastCharLength = [...match[0]][0]
                                        .length;
                                    const raw = src.slice(
                                        0,
                                        lLength +
                        ...
                        codespan(src) {
                            ...
                            if (cap) {
                                ...
                                const hasSpaceCharsOnBothEnds =
                                    this.rules.other.startingSpaceChar.test(
                                        text,
                                    ) &&
                         ...
                         */ inlineTokens(src, tokens = []) {
                            ...
                            while (
                                ...
                                maskedSrc =
                                    ...
                                    "]" +
                                    maskedSrc.slice(
                                        this.tokenizer.rules.inline.blockSkip
                                            .lastIndex,
                                    );
                     ...
                     */ class _Renderer {
                        ...
                        code({ text, lang, escaped }) {
                            ...
                            const code =
                                text.replace(other.endingNewline, "") + "\n";
                            if (!langString)
                                return (
                                    "<pre><code>" +
                        ...
                        tablecell(token) {
                            const content = this.parser.parseInline(
                                token.tokens,
                            );
                    ...
                    class Marked {
                        ...
                        use(...args) {
                            ...
                            args.forEach((pack) => {
                                ...
                                opts.async =
                                    this.defaults.async || opts.async || false;
                                ...
                                if (pack.renderer) {
                                    ...
                                    for (const prop in pack.renderer) {
                                        ...
                                        if (
                                            ["options", "parser"].includes(prop)
                                        )
                                            continue;
                                        const rendererProp = prop;
                                        const rendererFunc =
                                            pack.renderer[rendererProp];
                                        const prevRenderer =
                                            renderer[rendererProp];
                                        // Replace renderer with func to run extension, but fall back if false
                                        renderer[rendererProp] = (...args) => {
                                            let ret = rendererFunc.apply(
                                                renderer,
                                                args,
                                            );
                                            if (ret === false)
                                ...
                                if (pack.hooks) {
                                    const hooks =
                                        this.defaults.hooks || new _Hooks();
                                            ...
                                            hooks[hooksProp] = (arg) => {
                                                if (this.defaults.async)
                                                    return Promise.resolve(
                                                        ...
                                                        return prevHook.call(
                                                            hooks,
                                                            ret,
                                                        );
                                                    });
                                                const ret = hooksFunc.call(
                                                    hooks,
                                                    arg,
                                                );
                                                return prevHook.call(
                                ...
                                if (pack.walkTokens) {
                                    ...
                                    opts.walkTokens = function (token) {
                                        let values = [];
                                        values.push(
                                            packWalktokens.call(this, token),
                                        );
                                        if (walkTokens)
                                            values = values.concat(
                        ...
                        parseMarkdown(blockType) {
                            ...
                            const parse = (src, options) => {
                                ...
                                if (opt.async)
                                    return Promise.resolve(
                                        opt.hooks
                                            ? opt.hooks.preprocess(src)
                                            : src,
                                    )
                                        .then((src) => lexer(src, opt))
                                        .then((tokens) =>
                                            opt.hooks
                                                ? opt.hooks.processAllTokens(
                                                      tokens,
                                                  )
                                                : tokens,
                                        )
